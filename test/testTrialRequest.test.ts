    ...
    test("a good payload", () => {
        expect(
            async () => {
                ...
                await trialRequest.initialize(TRIAL_REQUEST_BODY)
            }
        ).not.toThrowError();
    });
...
describe("test JWT checking", () => {
    ...
    test("a valid JWT", async () => {
        ...
        await expect(request.fulfillAsync(basicTrial)).resolves.not.toThrowError();
    });
...
describe("test Express.js convenience methods", () => {
    ...
    test("it builds a trial from a request", async () => {
        ...
        const trialRequest = new TestBoxTrialRequest();
        await trialRequest.fromRequest(
            expressRequest
        );
        await expect(trialRequest.fulfillAsync(basicTrial)).resolves.not.toThrowError();
        expect(mock.history.post.length).toBe(1);
    });

    test("it will fail properly", async () => {
        const mock = new MockAdapter(axios);
        mock
            .onPost(
                "https://example.com/failure/c35dd919-6df3-49ca-96d0-d30e10dba442"
        ...
        await expect(trialRequest.reportFailureAsync(basicTrial)).resolves.not.toThrowError();
        expect(mock.history.post.length).toBe(1);
    });
